...
def movement(player):
    ...
    if len(pygame.sprite.spritecollide(player, players, False)) > 1 or pygame.sprite.spritecollide(player, walls,
                                                                                                   False):
...
def selectionScreen(thisStage):
    ...
    instructionP1 = pygame.image.load(
        'instruction2.png')  # pics with player controls, help users learn the controls of their tanks
    ...
    options = [imageUp_v1,
               imageUp_v2]  # actual options and their stats, this helps rendering/bliting when user switch the tanks
    stats = {0: ['3', '5', '3'], 1: ['5', '3', '1']}

    DisplaySurf = pygame.Surface((100, 100))
    DisplaySurf.fill((255, 255,
                      255))  # white surface, create contrast between tanks and background, easy for user to see tank differences
    ...
    while thisStage:
        ...
        212, 212, 212))  # these renders needs to be in the loop because as users switch
        p2statsHealth = my_font5.render('Health: ' + stats[Opt2][0], True,
                                        (212, 212, 212))  # between tanks, the stats info needs to update
